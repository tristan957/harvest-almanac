...
harvest_estimate_line_item_class_init(HarvestEstimateLineItemClass *klass)
{
	...
	obj_properties[PROP_KIND]		 = g_param_spec_string("kind", _("Kind"),
		   _("The name of an estimate item category."), NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT);
	obj_properties[PROP_DESCRIPTION] = g_param_spec_string("description", _("Description"),
		_("Text description of the line item."), NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT);
	obj_properties[PROP_QUANTITY]	= g_param_spec_int("quantity", _("Quantity"),
		   _("The unit quantity of the item."), 0, INT_MAX, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT);
	obj_properties[PROP_UNIT_PRICE]  = g_param_spec_double("unit_price", _("Unit Price"),
		 _("The individual price per unit."), 0, DBL_MAX, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT);
